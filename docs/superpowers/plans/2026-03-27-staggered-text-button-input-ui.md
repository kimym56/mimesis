...
Add assertions that button mode does not render the old `Text` label, keeps the overlay field structure inside the implementation shell, and uses an underline-style treatment with slightly larger type and a 50% width rule instead of the filled input background.
...
Keep the button preview markup label-free and leave the input in the existing overlay container. Restyle the overlay field with a transparent surface, light text, muted placeholder, slightly larger type, no full border, and a 50% width bottom rule with a restrained focus state.
