...
export function StaggeredTextButtonPreview({
  onTextChange,
  text,
  tuning,
}: {
  ...
  useEffect(() => {
    ...
    const animations = characterSlots.filter((slot) => !slot.isSpace).map((slot) => {
  ...
  return (
    <div className={`${styles.interactivePane} ${styles.buttonModePane}`} data-implementation="button">
      ...
      <p
        className={`${styles.previewInstruction} ${styles.previewInstructionButton}`}
      ...
      <button
        type="button"
        className={`${styles.trigger} ${styles.buttonStage}`}
